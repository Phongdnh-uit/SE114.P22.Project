...
public class CartItemServiceImpl implements CartItemService {
  ...
  public CartItemResponseDTO createCartItem(CartItemRequestDTO dto) {
    ...
    BigDecimal additionalPrice =
        newItem.getVariationOptions().stream()
            .map(
                vo ->
                    BigDecimal.valueOf(
                        vo.getAdditionalPrice() != null ? vo.getAdditionalPrice() : 0.0))
            .reduce(BigDecimal.ZERO, BigDecimal::add);
    newItem.setPrice(basePrice.add(additionalPrice));

    List<CartItem> existingItems =
        cartItemRepository.findAllByCartIdAndProductId(cart.getId(), newItem.getProduct().getId());
  ...
  public CartItemResponseDTO updateCartItem(Long id, CartItemRequestDTO dto) {
    ...
    BigDecimal additionalPrice =
        updated.getVariationOptions().stream()
            .map(
                vo ->
                    BigDecimal.valueOf(
                        vo.getAdditionalPrice() != null ? vo.getAdditionalPrice() : 0.0))
